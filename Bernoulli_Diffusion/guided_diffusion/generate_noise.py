...
def gen_noise(shape):
  ...
  #  if cfg.noisetype == 'simplex':
        ...
        #shape = (batchsize, 4,256,256)


        ns = generate_simplex_noise(simplex, torch.zeros(shape[0], shape[1], shape[2], shape[3]), 1, random_param=False).half()
        for i in range(shape[1]-1):
            N = generate_simplex_noise(simplex, torch.zeros(shape[0],shape[1],shape[2],shape[3]), 1, random_param=False).half()
            ns=torch.cat((ns,N), dim=1)
   # else:
