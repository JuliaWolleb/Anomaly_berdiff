...
class BinomialDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        print('modelmeantype', self.model_mean_type, self.loss_type)
    ...
    def p_mean(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(x, self._scale_timesteps(t), **model_kwargs)   #this is the residual epsilon
       # print('modeloutput', model_output.shape, model_output.max(), model_output.min())
      #  viz.image(model_output[0, 0, ...], opts=dict(caption="raw_" + str(t)))
      #  viz.image(x[0, 0, ...], opts=dict(caption="input_" + str(t)))
    ...
    def p_sample(
        ...
        if t[0] != 0:
            ...
            # return {"sample": sample, "pred_xstart": out["pred_xstart"]}
            return {"sample": out["mean"], "pred_xstart": out["pred_xstart"], "modeloutput": out["model_output"] }
    ...
    def p_sample_loop(
        ...
        """
        print('we are in the binomial p sample loop')
        final = None
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                yield out
                img = out["sample"]
                output=out["model_output"]
                if t%10==0:
                    viz.image(visualize(img[0, 0, ...]), opts=dict(caption="sample_"+str(i)))
                   # viz.image(visualize(output[0, 0, ...]), opts=dict(caption="output_"+str(i)))
    ...
    def ddim_sample(
        ...
        if t[0] != 0:
            ...
            Mask[0, mask2] = 1
            new_predxstart = out["pred_xstart"] * Mask.cuda() + org.cuda() * (abs(1 - Mask).cuda())

            model_mean = self.q_posterior_mean(
                    x_start=new_predxstart, x_t=x, t=t
                )
    ...
    def ddim_sample_loop(
        ...
        """
        final = None
        print('ended up in ddim sampling loop')
    ...
    def ddim_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            print('sampled image from binomial distribution')
            print('shape', shape)
