...
from utils.train_utils import visualize
...
class BinaryDiffusion(Sampler):
    ...
    def _train_loss(self, x_0, label=None, x_ct=None):
     ...
     #   print('loss during training', loss, kl_loss)
    ...
    def sample(self, temp=1.0, sample_steps=None, b=1, return_all=False, label=None, mask=None, guidance=None, full=False):
        device = 'cuda'
        ...
        if self.dataset == 'imagenet' or self.dataset == 'chexpert':
            if label is None:
        ...
        print('got guidance', guidance)
        for i, t in enumerate(sampling_steps):
            t = torch.full((b,), t, device=device, dtype=torch.long)

            if self.dataset.startswith('imagenet') or self.dataset.startswith('laion') or self.dataset.startswith('chexpert'):
                x_0_logits = self._denoise_fn(x_t, label, time_steps=t-1)
                print('x0logits', x_0_logits.shape)
            ...
            print('x0logits_second', x_0_logits.shape)
    ...
    def loop(self, x, level, temp=1.0, sample_steps=64, b=1, return_all=False, label=None, mask=None, guidance=None,
               full=False):
        device = 'cuda'
        x_0 = x
        x_t = self.q_sample(x_0, level)

        xta = x_t
        print('xta', x_t.shape)

        x_t_in = x_t #torch.bernoulli(x_t)
        print('x0', x_0.shape, x_0.max(), x_0.min())
        print('added noise x_t', x_t_in.shape, x_t_in.max(), x_t_in.min())

        #x_t = torch.bernoulli(0.5 * torch.ones((b, np.prod(self.shape), self.codebook_size), device=device))
        print('random x_t', x_t.shape, x_t.max(), x_t.min())

        if mask is not None:
            m = mask['mask'].unsqueeze(0)
            latent = mask['latent'].unsqueeze(0)
            x_t = latent * m + x_t * (1 - m)
        sampling_steps = np.array(range(1, level + 1))  #self.num_timesteps

        # if sample_steps != level:
        #     idx = np.linspace(0.0, 1.0, sample_steps)
        #     idx = np.array(idx * (self.num_timesteps - 1), int)
        #     sampling_steps = sampling_steps[idx]
        ...
        sampling_steps = sampling_steps[::-1]
        print('anzahl sampling steps', sampling_steps)
